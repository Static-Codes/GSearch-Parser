...
def initialize_browser(use_proxy: bool = False, proxy_type: Optional[ConfigProxyType] = None, user_agent: Optional[str] = None) -> Optional[Driver]:
    ...
    try:
        ...
        browser = Driver(uc=True, protocol=proxy_type, proxy=PROXY, )
        #browser.set_window_rect(x=0, y=1920, width=1920, height=1080)
...
def proxy_works_in_browser(driver: Driver) -> bool:
    """
...
def render_html(driver: Driver) -> str:
    try:
...
def page_table_present(driver: Driver) -> bool:
    try:
...
def all_results_present(driver: Driver) -> Union[str, bool]:
    try:
...
def captcha_onload(driver: Driver) -> Union[str, bool]:
    try:
...
def captcha_missing(driver: Driver) -> bool:
    try:
        time.sleep(3)
...
def swap_proxy(driver: Driver, PROXY_TYPE: ConfigProxyType = ConfigProxyType.HTTP, retries: int = 10, current_url: str = None) -> Union[bool, Driver]:
    """
...
def get_captcha_url(driver: Driver) -> Optional[str]:
...
def get_audio_link(driver: Driver) -> Optional[str]:
    try:
...
def get_search_response(driver: Driver, search_link: str, thread_id: str) -> Optional[str]:
    location = get_error_location()
...
def click_more_results(driver: Driver, thread_id: str) -> Union[bool, str]:
    try:
...
def close_browser(driver: Driver) -> None:
    try:
        driver.quit()
    except Exception as e:
        location = get_error_location()
